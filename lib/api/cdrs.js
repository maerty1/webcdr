...
router.get('/cdrs', function (req, res) {
  ...
  var filter = function () {
    this.where(function () {
	  ...
	  .orWhere('cnum', 'like', like)
          .orWhere('dst', 'like', like);
	  .orWhere('did', 'like', like);
      } else {
        this.whereRaw('1=1');
      }
    }).andWhere(function () {
      ...
      if (req.query.direction) {
        var sqlDirection = req.query.direction.map(function (d) {
          switch (d) {
            case 'in': return '(LENGTH(src) > 5 AND LENGTH(dst) <= 5 OR LENGTH(did) > 5)';
            case 'out': return '(LENGTH(src) >= 1 AND LENGTH(src) <= 5 AND LENGTH(dst) > 5 OR LENGTH(CNUM) >= 1 AND LENGTH(CNUM) <= 5 AND LENGTH(dst) > 5)';
      ...
      if (req.user.acl) {
        this.where(function () {
          ...
          this.whereIn('cnum', req.user.acl);
        }).orWhere(function () {
          this.whereIn('dst', req.user.acl);
        });
        if (req.user.acl_in) {
          // allow to see inbound calls in addition to ACLed numbers
          this.orWhere(function () {
            this.whereRaw('(LENGTH(src) > 5 OR LENGTH(CNUM) > 5 AND LENGTH(dst) <= 5 OR LENGTH(did) > 5)');
          });
        }
      } else {
        this.whereRaw('1=1');
